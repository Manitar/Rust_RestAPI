...
fn fetch_all_todo_items() -> Result<Json<ToDoList>, String> {
    ...
    let mut /*changeble */ statement = match db_connection.prepare("select id, item from todo_list;") { //get id,item for table todo_list
...
fn add_todo_item(item: Json<String>) -> Result<Json<StatusMessage>, String> { 
    let db_connection = match Connection::open("data.sqlite") { //connect
